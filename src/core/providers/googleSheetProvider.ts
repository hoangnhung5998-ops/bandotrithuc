...
export class GoogleSheetProvider implements DataProvider {
  ...
  async sync(): Promise<void> {
    ...
    try {
      ...
      const uniqueTargets = Array.from(new Set(resourcesToSync));
  ...
  async create<T>(resource: string, data: any): Promise<T> {
    ...
    // Gửi lệnh về Google Sheets ngầm bên dưới
    this.callApi('UPSERT', target, rawData).catch(err => {
      console.error(`[GoogleSheetProvider] Background create failed for ${resource}`, err);
      // Có thể rollback cache ở đây nếu cần thiết
    });
  ...
  async update<T>(resource: string, id: string, data: any): Promise<T> {
    ...
    updateTask().catch(err => {
      console.error(`[GoogleSheetProvider] Background update failed for ${resource}:${id}`, err);
    });
    
    // Trả về merged từ cache ngay lập tức
    return (merged || { ...data, id }) as T;
  }
  ...
  async delete(resource: string, id: string): Promise<void> {
    ...
    deleteTask().catch(err => {
      console.error(`[GoogleSheetProvider] Background delete failed for ${resource}:${id}`, err);
    });
  }
  ...
  async getClassReport(classId: string): Promise<ClassReport> {
    // This is a complex aggregation, for now return mock or implement server-side in Apps Script
    return {
      totalStudents: 35,
      totalLessons: 12,
      completedLessons: 8,
      ...
      studentScores: [],
      progressStats: { completed: 25, inProgress: 8, notStarted: 2 },
      submissionStats: { onTime: 30, late: 3, notSubmitted: 2 }
    };
  }
