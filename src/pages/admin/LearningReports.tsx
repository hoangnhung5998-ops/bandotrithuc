...
import { User, Class, Lesson } from '../../core/types';
...
export const LearningReports = () => {
  ...
  const fetchData = async () => {
    const allUsers = await dataProvider.getList<User>('users');
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="mt-4 flex items-center gap-1 text-xs text-emerald-500 font-bold">
            <TrendingUp size={14} /> +5% so với tuần trước
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-sm text-slate-500 mb-1">Bài học phổ biến nhất</p>
          <p className="text-lg font-bold text-slate-800 line-clamp-1">Văn minh Sông Hồng</p>
          <p className="mt-4 text-xs text-slate-400">85% học sinh đã học</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-sm text-slate-500 mb-1">Quiz khó nhất</p>
          <p className="text-lg font-bold text-slate-800 line-clamp-1">Địa lý vùng cao</p>
          <p className="mt-4 text-xs text-slate-400">Điểm TB: 6.5/10</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-sm text-slate-500 mb-1">Tổng số giờ học</p>
          <p className="text-3xl font-bold text-blue-600">1,240h</p>
