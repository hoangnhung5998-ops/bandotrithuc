          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {(formData.options || []).map((opt, idx) => (
                <div key={`mc-${idx}`} className="flex items-center gap-3">
                  <input 
                    type="radio" 
                    name="correctAnswer" 
      ...
      case 'sorting':
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {(formData.options || []).map((opt, idx) => (
                <div key={`sort-${idx}`} className="flex items-center gap-3">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {(Array.isArray(formData.correctAnswer) ? formData.correctAnswer : []).map((ans, idx) => (
                <div key={`fill-${idx}`} className="flex items-center gap-3">
                  <span className="text-xs font-bold text-slate-400 w-4">{idx + 1}.</span>
                  <input 
                    type="text" 
                    value={ans} 
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {(formData.options || []).map((opt, idx) => (
                <div key={`match-${idx}`} className="grid grid-cols-12 gap-3 items-center">
