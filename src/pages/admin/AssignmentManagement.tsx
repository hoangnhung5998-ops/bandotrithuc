...
export const AssignmentManagement = () => {
  ...
  const handleSaveGrade = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Re-fetch data to ensure sync with backend
      await fetchData();
      
      setIsGradeModalOpen(false);
      // Removed the setTimeout(() => fetchData(), 1000) to prevent overwriting optimistic state
    } catch (error) {
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredSubmissions.map((submission) => {
                  ...
                  const isGraded = submission.score !== undefined && 
                                  ...
                                  submission.score !== '' &&
                                  submission.score !== '--';
                  // Nếu score là số 0, nó vẫn được coi là đã chấm
                  const isGradedFinal = isGraded || submission.score === 0;
                    ...
                    <tr key={submission.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold ${
                          isGradedFinal ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                        }`}>
                          {isGradedFinal ? <CheckCircle size={12} /> : <Clock size={12} />}
                          {isGradedFinal ? 'Đã chấm' : 'Chưa chấm'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        {isGradedFinal ? (
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          >
                            {isGradedFinal ? 'Xem lại' : 'Chấm bài'} <ChevronRight size={14} />
                          </button>
                          <button 
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {ensureArray<any>(formData.rubrics).map((rubric, index) => (
                      <div key={rubric.id || index} className="flex items-center gap-3">
                        <input 
                          required
                          type="text" 
